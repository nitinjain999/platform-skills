...
Rewrites `/platform-skills:triage` for reliability, the first of two capability-reliability passes in this release — replaces every hand-rolled `gh api`/jq call behind the command with a 12-subcommand stdlib-only helper CLI, splits finding classification from fix-execution and discussion state, and adds `--dry-run`/`--no-resolve` controls.
...
- **GraphQL calls lost type information** — the previous flow built every GraphQL query with inline `-f`/`-F` fields (`-f owner=<owner> -F pr=<pr_number>`); `--input` payload files replace that for every GraphQL call (thread listing, reply, resolve), so an integer field like a PR number can no longer silently arrive at the API as a string
