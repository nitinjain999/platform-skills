...
- **`.github/workflows/zizmor.yml`** — this repository now audits its own GitHub Actions definitions on every relevant PR and push to `main`, via `zizmorcore/zizmor-action` v0.6.2 with zizmor pinned to 1.29.0. Scope is the whole repo: `.github/workflows/**` plus the composite actions and example workflows under `examples/`, since users copy those. One zizmor run drives three outputs — a single sticky PR comment (severity table split into "this repo's CI" vs "shipped examples", high/medium listed with docs links, long tail left in the Security tab), a SARIF upload to Security → Code scanning under category `zizmor`, and the job summary. A first run reported 231 findings (31 high, 18 medium); those were fixed in the same release, so the gate ships at `ZIZMOR_FAIL_ON: high` and blocks only on new regressions. It is not at `medium` yet, and the reason is not obvious: `artipacked` reports low online but medium offline, because it downgrades when `actions/checkout` is v6.0.0+ and resolving a SHA pin to a version needs the network — so `medium` would pass online and fail every PR the moment the token is missing or rate-limited
