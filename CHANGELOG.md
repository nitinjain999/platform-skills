...
- `commands/karpenter.md`: new `/platform-skills:karpenter` command with six modes — `generate` (EC2NodeClass + NodePool from workload requirements, with environment wizard asking EKS version, identity method, Spot tolerance, private cluster, and GitOps method), `debug` (evidence-first diagnosis of pending pods and stuck NodeClaims via decision tree), `review` (correctness / cost / reliability / security / GitOps safety priority-ordered review), `audit` (reconstruct scale-out/scale-in timeline from NodeClaim events, disruption-reason annotations, and controller logs), `migrate` (CA → Karpenter pre-migration checklist + cordon/drain sequence with blast radius), and `upgrade` (patch/minor + breaking v0.x → v1.x CRD migration with field-mapping table).
