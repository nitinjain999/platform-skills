...
- **`.github/workflows/zizmor.yml`** — this repository now audits its own GitHub Actions definitions on every relevant PR and push to `main`, via `zizmorcore/zizmor-action` v0.6.2 with zizmor pinned to 1.29.0. Scope is the whole repo: `.github/workflows/**` plus the composite actions and example workflows under `examples/`, since users copy those. One zizmor run drives three outputs — a single sticky PR comment (severity table split into "this repo's CI" vs "shipped examples", high/medium listed with docs links, long tail left in the Security tab), a SARIF upload to Security → Code scanning under category `zizmor`, and the job summary. Deliberately **advisory** at launch (`ZIZMOR_FAIL_ON: none`): a first run reports 231 findings (31 high, 18 medium), so a hard gate would fail every PR regardless of its contents. Raise the knob to `high` once `.github/workflows` is clean
