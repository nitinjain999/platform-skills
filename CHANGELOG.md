...
- **`publish` had no guard against a stale or moved remote head** — the previous flow ran a bare `git push` with no pre-push recheck of the PR's actual head SHA and no post-push confirmation that the remote head matched what was sent. `publish` now rereads the PR head immediately before pushing and refuses with `HEAD_MOVED` if it no longer matches the plan's expected head, pushes an explicit `<sha>:refs/heads/<ref>` refspec instead of a bare push, classifies a rejected push by cause (`PUSH_REJECTED_NON_FASTFORWARD`, `PUSH_REJECTED_BY_POLICY`, `NO_PUSH_PERMISSION`, or `UNKNOWN_TRANSPORT_FAILURE`) instead of a raw exit code, and confirms the push against two independent reads before the caller may report `matches_pushed_commit: true`: `git ls-remote` against `--head-remote-url` (`remote_head_after`) and a fresh `repos/{repo}/pulls/{pr}` read of the PR's own head (`pr_head_after`). A wrong `--head-remote-url` still pushes somewhere real, and `ls-remote` against that same wrong destination still agrees with itself, so only `pr_head_after` proves the PR moved. That second read sits after an irreversible mutation and therefore cannot fail the call: when it errors (rate limit, 502, replication lag) the response is still `ok: true` with `push_landed: true`, `pr_head_after: null`, `verification_incomplete: true`, and `matches_pushed_commit: false`, so "the push failed" and "the push succeeded and could not be confirmed against the PR record" stay two different facts. The second must not be retried, since a retry returns `HEAD_MOVED`
...
- **`stage-commit` could not see what a commit hook changed** — the file-set allowlist check ran before `git commit`, so a `pre-commit` hook that staged an extra file, dropped one, or reformatted an intended file in place produced a commit that no longer matched what was validated, reported as a success. It now records each intended path's staged blob hash immediately before committing and rereads the sealed commit, raising `COMMIT_FILE_SET_DRIFTED_FROM_STAGED` (with `committed`, `intended`, `commit_sha`) when the commit's file set moved and `COMMIT_CONTENT_DRIFTED_FROM_STAGED` (with `drifted_paths`, `commit_sha`) when the set is identical but a hook rewrote an intended file's bytes. A formatter hook produces the second, which a file-set comparison alone cannot detect. `committed_paths` in the success response is read back from the sealed commit (`git diff-tree --root`), not the pre-commit index
