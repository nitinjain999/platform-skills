...
- **Bounded recovery** — at most one denial per target per session. The core writes the denied file path to `.token-optimizer/state/<session_id>/<target_hash>`. The second attempt on the same file passes. No denial at all when the attempt cannot be persisted, since unlogged enforcement is misleading
