...
- `commands/gitops-audit.md` — new slash command `/platform-skills:gitops-audit`. Six-phase read-only audit workflow: Phase 1 Discovery (inventory resources by kind, classify repo pattern: basic-monorepo/multi-repo fleet/fleet-with-resourcesets/gitless, check for `gotk-sync.yaml` → migration flag), Phase 2 Manifest Validation (YAML syntax, `kustomize build` per overlay, `kubeconform -strict -summary`, skip rules for SOPS secrets / third-party CRDs / `${VARIABLE}` substitution / `gotk-components.yaml`), Phase 3 API Compliance (`flux migrate --dry-run`, deprecated HelmRelease/Kustomization/source apiVersions), Phase 4 Best Practices (prune/wait/timeout/dependsOn on Kustomizations; RetryOnFailure/drift detection/semver ranges on HelmReleases; `reconcile.fluxcd.io/watch: Enabled` on `valuesFrom` ConfigMaps; ResourceSet `<< >>` delimiters; OCIRepository `layerSelector.mediaType`), Phase 5 Security Review (SOPS/ESO for secrets, Workload Identity over static credentials, Cosign OCI verification, multi-tenant RBAC isolation, image automation credential separation), Phase 6 Report (Summary / Directory Structure / Validation Results / API Compliance / Best Practices / Security / Recommendations — Critical / Warning / Info).
- `examples/flux/multi-tenant/` — multi-tenant repo pattern: `clusters/production/tenants.yaml`, `tenants/team-a/` (namespace, ServiceAccount in flux-system, RoleBinding to tenant-reconciler ClusterRole, GitRepository, Kustomization with `serviceAccountName` and `targetNamespace` for RBAC isolation), `platform/rbac/tenant-role.yaml` (ClusterRole scoped to workload resources), `platform/network-policy/default-deny.yaml` (default-deny-all + allow-same-namespace + DNS egress per tenant namespace). README includes bootstrap instructions, troubleshooting, and RBAC can-i verification.
- `examples/flux/helm-releases/` — Helm chart management via OCI: `releases/base/cert-manager/ocirepository.yaml` (semver `1.x`, `layerSelector.mediaType`), `releases/base/cert-manager/helmrelease.yaml` (`spec.chartRef`, `RetryOnFailure`, `driftDetection.mode: enabled`, `valuesFrom`), `releases/base/cert-manager/values-configmap.yaml` (`reconcile.fluxcd.io/watch: Enabled`), `releases/staging/kustomization.yaml` (1 replica overlay), `releases/production/kustomization.yaml` (3 replicas, PDB, anti-affinity). README covers key patterns and common mistakes.
- `examples/flux/image-automation/` — both image automation models side-by-side: `git-based/imagerepository.yaml`, `git-based/imagepolicy.yaml` (semver range), `git-based/imageupdateautomation.yaml` (staging branch push, not main); `gitless/resourcesetinputprovider.yaml` (OCIArtifactTag, semver filter), `gitless/resourceset.yaml` (`<< inputs.tag >>` substitution). README includes comparison table, setup steps for both models, safety rules, and troubleshooting.
...
- `SKILL.md` — added Domain 29 `GitOps Audit`; updated Flux reference description to include Flux Operator, FluxInstance, ResourceSet, gitless delivery, source selection, and common mistakes; added `/platform-skills:gitops-audit` slash command.
...
- `GETTING_STARTED.md` — updated to "All 29 command workflows"; added `gitops-audit` to command table.
