...
Every file written by setup-agents carries a managed-file marker so upgrade and add can detect whether a file is safe to overwrite. The syntax — and the line it appears on — is format-dependent:

| Target file | Line | Marker |
|-------------|------|--------|
| `.github/agents/*.agent.md` | 2 (inside frontmatter, YAML comment — `---` must stay at byte 0 for Copilot's frontmatter parser) | `# generated by platform-skills` |
| `.cursor/rules/*.mdc` | 1 | `<!-- generated by platform-skills -->` |
| `AGENTS.md`, `.windsurfrules` | 1 | `<!-- generated by platform-skills -->` |
| `agents/openai.yaml` | 1 | `# generated by platform-skills` |
...
This check reads the first three lines (not just line 1) so it catches the `.agent.md` marker on line 2, and works across both comment styles (`<!--` and `#`) because it greps for the common substring.
