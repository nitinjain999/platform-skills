...
Valid manifest tokens: `copilot-vscode`, `copilot-cloud`, `copilot-app`, `cursor`, `claude`, `codex`, `windsurf`, `vscode-mcp`. One token per line. Lines starting with `#` are comments. Do not add duplicate tokens — check first:
...
grep -q '## Agent Context' CLAUDE.md 2>/dev/null && echo "claude" >> .platform-skills/manifest
