...
**Verified against `examples/triage/scripts/triage_helper.py` at commit `bae2362`** (11 top-level subcommands, state schema version 1). `worktree` and `state` each nest further verbs (`worktree prepare`/`cleanup`/`preserve`; `state lock`/`unlock`/`read`/`write`), for 16 invokable operations in total if you count every leaf individually. If the installed helper's `--help` output disagrees with a flag shown here, trust the installed helper and treat this file as stale for that detail.
...
- **It cannot coordinate across machines.** `state lock` is a single local file with `O_CREAT|O_EXCL`, carrying a random `lock_token` only the acquiring run is handed. It stops two runs on the same checkout from racing, and the token is what stops the second one from releasing the first one's lock. It does nothing for a second checkout on a different machine or CI runner; the remote-state rereads in Phases B, F, and G are what actually protect against that.
...
`snapshot` does the pagination work so nothing downstream has to: it walks `reviewThreads` with its own outer cursor, then walks each thread's `comments` with its own inner cursor, and only stops when every page of both connections has been consumed. Calling it a second time for the same run does not save you from anything and can actively hurt: it queries live GitHub state again, and that state can have moved — a new comment, an edit, a resolution someone else made — so a second call is not a cache refresh of the first, it is new evidence that may disagree with it. Phase B happens exactly once per run for that reason: one authoritative snapshot per analysis attempt. If later evidence shows that snapshot is stale, that calls for a fresh attempt with its own reassessment, not a silent re-snapshot mid-attempt — see the recovery guidance further down for what "fresh attempt" means in practice.
...
Hooks do not run on this path at all. `stage-commit` passes `-c core.hooksPath=<an empty directory>` to both its `git add` and its `git commit`, so a repository `pre-commit`, `prepare-commit-msg`, `commit-msg`, or `post-commit` hook never executes, and `worktree prepare` does the same for its `fetch` and `worktree add`. An earlier revision deliberately let the commit's hooks run, on the theory that a legitimate formatter hook was worth allowing because the drift checks below would observe whatever it changed. That traded the wrong thing away: a hook is arbitrary code running with this run's credentials, and one that makes a network call, reads an unrelated file, or writes outside the staged paths changes no staged bytes at all, so every drift check sees a clean commit and reports success while the side effect goes unrecorded and unblocked. Hook-driven validation is not forbidden as an idea; it is forbidden as an implicit consequence of committing. If a repository's formatter or linter genuinely has to run as part of a fix, invoke it deliberately as its own Phase E step and revalidate the result, where the agent can see it happen.

The allowlist check above still runs before the commit, and two more checks still run after it, now as defense in depth rather than as a hook watchdog. `stage-commit` records each intended path's staged blob hash (`git rev-parse :<path>`) immediately before committing, then rereads the sealed commit: `COMMIT_FILE_SET_DRIFTED_FROM_STAGED` if the commit's file set (`git diff-tree --root <sha>`) no longer equals the intended paths, and `COMMIT_CONTENT_DRIFTED_FROM_STAGED` if the file set is identical but any intended path's blob hash moved. With hooks disabled neither should ever fire; if one does, something else moved the index between the check and the commit, and the right response is to reinspect the commit rather than to assume the check is spurious. The second check is the one a file-set comparison alone cannot see, since content movement changes bytes, not filenames.
...
Only a reported PASS unlocks `stage-commit` (Phase D). Commit hooks cannot interfere with what that PASS describes, because `stage-commit` disables them for both its `add` and its `commit` (Phase D above explains why that is a security decision, not a convenience one). If a repository's formatter or linter has to run, run it here, explicitly, before `stage-commit`, and treat its output as part of the change being validated. If the two post-commit drift checks (`COMMIT_FILE_SET_DRIFTED_FROM_STAGED`, `COMMIT_CONTENT_DRIFTED_FROM_STAGED`) ever fire anyway, **re-inspect the resulting commit and revalidate before publication** — rerun the relevant tests, linters, or render checks against the tree that was actually committed. The code can prove that something drifted; only a human or the agent can decide whether the drifted result is still correct, and no hash comparison will ever answer that.

---

## Publish & reconcile (Phase F)

`publish` reads `repos/{repo}/pulls/{pr}` once before it pushes anything, and that single read feeds five refusals. All five raise before the `git push` subprocess is ever invoked, which is the whole point: a push is irreversible, so every destination question has to be settled while it is still a question.

| Refusal | Condition | Why it is a preflight and not a report |
|---|---|---|
| `PR_READ_UNUSABLE` | The pre-push read did not return a JSON object | Every refusal below is derived from that one read; if it is unusable, nothing has been verified at all |
| `PR_NOT_OPEN` | The PR's `state` is not `open` | A closed or merged PR is not a publication target; pushing to its branch is not "still fine" |
| `HEAD_MOVED` | `head.sha` no longer equals `--expected-head-sha` | The plan was built against a tree that is no longer the head; revalidate rather than push over it |
| `HEAD_REPO_UNAVAILABLE` | `head.repo` is `null` | The fork was deleted or is inaccessible, so there is no identity to verify a destination against at all |
| `PUBLISH_DESTINATION_MISMATCH` | `--head-remote-url` resolves to a different host or a different `owner/repo` than `head.repo.clone_url`/`head.repo.full_name`, or `--head-ref` is not `head.ref` | The push would land somewhere the PR does not point at, and a mutation cannot be taken back once it lands |

`PUBLISH_DESTINATION_MISMATCH` closes a gap the SHA check alone could not: `--expected-head-sha` compares a SHA, and a SHA says nothing about *which repository* the URL in argv points at. Two repositories seeded from the same commit have the same head SHA, so a caller error — or a wrong URL suggested by untrusted comment text — passed the old preflight untouched and only surfaced afterwards as `matches_pushed_commit: false`, by which time an unrelated repository had already been mutated.

The comparison is on **host and `owner/repo` together**, because the trailing path segments alone identify nothing: `https://evil.example.com/acme/widgets.git`, `git@evil.example.com:acme/widgets.git`, `file:///tmp/x/acme/widgets`, and a bare `../acme/widgets` all carry the same `acme/widgets` tail as the real head repository on `github.com`, so an owner/repo-only check would have passed every one of them. `--head-remote-url` and the PR's own `head.repo.clone_url` (from the same single pre-push read, no extra API call) are each split into a host and a path: the `https://`/`ssh://` forms through `urllib.parse.urlsplit`, the scp-like `git@host:owner/repo` form explicitly, with or without a trailing slash or `.git`, and userinfo (`x-access-token:<token>@`) excluded from the host. Both halves must agree, case-insensitively.

A URL with no host at all — a local path, a `file://` URL, a relative path — is a mismatch against a hosted head repository rather than a pass, because "the host cannot be determined" is not evidence that it is the right host. Two hostless paths match only when they resolve to the same filesystem location, so `/srv/a/acme/widgets.git` and `/srv/b/acme/widgets.git` are correctly different destinations. A `head.repo.clone_url` that is missing or empty also fails closed. A URL that yields no `owner/repo` identity at all is a mismatch too, and that shape check now requires exactly two non-empty path segments, so `https://github.com/acme` (an owner with no repository) yields no identity instead of the host-plus-owner pair `github.com/acme` it used to produce. The error carries `expected_repo`, `given_repo`, `expected_host`, `given_host`, `expected_ref`, `given_ref`, and redacted `given_url`/`expected_url` so the caller can see exactly which half disagreed.

The `HEAD_MOVED` check remains a courtesy, not a server-side compare-and-swap guarantee. A non-forced push prevents overwriting a divergent branch, but a branch that moves backward or sideways between the read and the push is a subtler race this check does not close. Do not describe this as atomic head locking, and never force-push to keep an old plan moving anyway; if strict expected-old-SHA enforcement is ever required, that is a separate, deliberately designed and tested mechanism, not something to fake here.

A failed push is classified, not just surfaced raw, and the order of those checks matters because one rejection message can match several patterns at once — and `git push` stderr always echoes the remote URL and ref name, so a branch or repo name that happens to contain a word like "permission" or "403" must not steer a genuine race into the wrong bucket. `PUSH_REJECTED_NON_FASTFORWARD`'s unambiguous tokens ("non-fast-forward", "fetch first") are tested first for exactly that reason. Then `PUSH_REJECTED_BY_POLICY` ("protected branch", "hook declined"): the head repository refused the push on purpose, so refreshing and retrying will not clear it and force-pushing is not an option either. Then `NO_PUSH_PERMISSION` (no write access to the head repository; the fork-without-write-access case from Phase A, not something to route around). Only after all three of those does a bare "rejected" fall back to `PUSH_REJECTED_NON_FASTFORWARD` (someone else genuinely moved the head; refresh and revalidate before retrying, never force) — that loosest check runs last so it never shadows the more specific ones above it. Anything unmatched is `UNKNOWN_TRANSPORT_FAILURE`. A successful push is confirmed by rereading two independent sources afterward: `git ls-remote` against `--head-remote-url` itself (`remote_head_after`), and a fresh `repos/{repo}/pulls/{pr}` read of the PR's actual head (`pr_head_after`). `matches_pushed_commit: true` requires both to equal `--commit-sha`, not just the first; `ls-remote` reads back the same URL the push used, so it agrees with itself by construction and only `pr_head_after` proves the PR record itself moved. Treat anything else, including a push command that exited zero but either reread disagreeing, as unconfirmed.
...
The literal suffix `✅ Fixed` is reserved for a reply whose fix was actually verified published (`publish` returned `matches_pushed_commit: true`) **and** whose required checks are not failing — a required check that is still pending does not by itself disqualify the suffix as long as the reply says so, but a failing check, or a required-check state that could not be discovered at all, does. It must be the last thing in the body with nothing after it; if a dedup marker needs to be in the body too, it goes earlier, never after the suffix. A failed, local-only, or unvalidated change must never carry that suffix. An informational reply should state what actually happened, not default to "no change needed" when that is not true.
...
Cleanup runs regardless of how the run ended, and its order is load-bearing: `worktree preserve`, then `state write`, then `worktree cleanup`, then `state unlock`. `worktree cleanup` calls `git worktree remove --force`, which deletes that worktree's `.git/worktrees/<id>` together with its HEAD and reflog — so a commit that only ever existed there becomes unreachable and collectable — and deletes the working directory itself, taking every untracked file with it.

On any path that did not end in a confirmed publish, `worktree preserve --path <dir> --original-head-sha <prepared_head_sha> --out <a temp dir the agent creates>` captures all three channels of work in one call: `committed.patch` (`git format-patch <original>..HEAD --stdout`) for anything committed past the prepared head, `uncommitted.patch` (`git diff HEAD --binary`) for edits on top of that, and byte-for-byte copies under `untracked/` of every file `git status --porcelain -uall` reports as untracked. The agent records the returned paths in the `state write` record.

This used to be prose telling the agent to run `git format-patch` or `git diff HEAD` by hand, and that prose was unsafe in a specific, silent way: **`git diff HEAD` never shows untracked files.** A brand-new implementation file or a brand-new regression test that was written but never `git add`ed appears in no diff at all, so a recovery instruction built on `git diff HEAD` captured nothing of it, and the forced removal that followed deleted it permanently with no error, no warning, and no way to notice until the work was already gone. Preservation therefore had to become code that also verifies its own output rather than an instruction the agent is trusted to have satisfied.

`worktree preserve` verifies before it reports success: if HEAD moved past `--original-head-sha` but no patch could be produced, if any copied untracked file's size does not match its source, or if the set of copied paths does not cover every path `git status` reported as untracked, it raises `PRESERVATION_FAILED` with a `problems` list and leaves the worktree untouched. That last comparison is what makes a skipped entry visible: an entry the copy could not handle (an unreadable file, a nested repository) would otherwise vanish from the output while the response still said `ok: true`, immediately before the forced removal deleted the original. Symlinks are preserved as symlinks (`shutil.copy2(..., follow_symlinks=False)`), including a symlink to a directory and a dangling one, since dereferencing a link is a different artifact and skipping it is a silent loss. **On `PRESERVATION_FAILED` the agent must not call `worktree cleanup` at all** — it stops, reports the block, and leaves the worktree in place for manual recovery. Losing the run is recoverable; losing the work is not. Getting this order or this gate wrong is what silently turns the preservation promises in the recovery table below into a lie.

The lock is released even when the run ends early or blocked, with the `lock_token` the acquire returned; a run that dies mid-flight must never leave a stale lock behind for the next invocation. When one does survive anyway, recovery is deliberate rather than automatic: see the lock section in the data model below.
...
Mutual exclusion needs a release that proves ownership, or it is only half a lock. The acquire generates a random `lock_token` (`secrets.token_hex(16)`), stores it in the lock file next to `pid`/`acquired_at`, and returns it in the `ACQUIRED` response. That token belongs in the caller's own run-scoped context, never persisted anywhere on the filesystem where a second run could read it back — the whole guarantee is that only the run that acquired the lock holds it. A normal `state unlock` requires a matching `--lock-token` and refuses with `LOCK_TOKEN_MISMATCH` otherwise. Without that check, run B could call plain `state unlock` and remove run A's active lock, after which both runs would happily mutate the same PR, which is precisely the outcome the lock exists to prevent.
...
`--force-unlock` removes the lock file unconditionally and is the only bypass; there is deliberately no age-based automatic bypass, because `age_seconds` is input to a human or agent judgment, never a substitute for holding the token. Without `--force-unlock`, `state unlock` releases a lock only when `--lock-token` matches the token recorded in the file (`LOCK_TOKEN_MISMATCH` when it does not, including when no token was supplied at all), and refuses a lock file it cannot recognise as its own — no readable `pid` and no readable `lock_token` — with `LOCK_NOT_RECOGNIZED` rather than deleting something it does not understand. Unlocking when nothing is held is not an error: it returns `RELEASED` with `existed: false`.

`state`'s four verbs all resolve paths inside `<repo-root>/.git/triage-state/`, which requires `.git` to be a real directory. Point `--repo-root` at a linked worktree (where `.git` is a file containing a `gitdir:` pointer) and every one of them fails fast with `REPO_ROOT_IS_LINKED_WORKTREE` naming the path, instead of an unexplained `NotADirectoryError`. That check runs on the pure path resolution, so `state read` and `state unlock` still report it without creating anything. `REPO_ROOT` is the caller's main checkout, never the Phase D worktree; this error is what enforces that.
...
"Preserve" is an instruction with a specific mechanic behind it, not a hope. Nothing preserves itself: the first three rows all describe state that lives only inside the Phase D worktree, and `worktree cleanup` destroys it. Run `worktree preserve --path <worktree> --original-head-sha <prepared_head_sha> --out <temp dir>` and put its returned paths in the `state write` record *before* cleanup, on every failure path — it covers the commit, the uncommitted edit, and the untracked file in one verified call, which is exactly what a hand-run `git diff HEAD` did not. If it raises `PRESERVATION_FAILED`, skip cleanup entirely and report the block. A row in this table that was never captured that way is just a promise the run did not keep.
...
Run this on every path that did not end in a confirmed publish, immediately before `state write` and therefore before `worktree cleanup`:

```bash
python3 "$CLAUDE_PLUGIN_ROOT/examples/triage/scripts/triage_helper.py" worktree preserve \
  --path /tmp/triage-worktree-a1b2c3 \
  --original-head-sha 9f2a1c4e8b3d5f60a1c2b3d4e5f60718293a4b5c \
  --out /tmp/triage-482-preserved
```

```json
{
  "ok": true,
  "out_dir": "/tmp/triage-482-preserved",
  "committed_patch": "/tmp/triage-482-preserved/committed.patch",
  "uncommitted_patch": "/tmp/triage-482-preserved/uncommitted.patch",
  "untracked_files": ["tests/test_worker_timeout.py"]
}
```

Three independent channels, because no single `git` read covers all three. `committed.patch` is `git format-patch <original-head>..HEAD --stdout` and is `null` when HEAD never moved past `--original-head-sha`. `uncommitted_patch` is `git diff HEAD --binary` and is `null` when the tree is clean relative to HEAD; the `--binary` flag is load-bearing, since a plain `git diff` emits `Binary files ... differ` and produces a patch `git apply` will reject. `untracked_files` lists paths copied byte-for-byte into `<out>/untracked/<path>`, read from `git status --porcelain -uall`; this is the channel a hand-run `git diff HEAD` never had, and the reason a new-but-unstaged implementation or test file used to be destroyed silently by the cleanup that followed.

Applying `committed.patch` (with `git am`) then `uncommitted.patch` (with `git apply`) against a fresh checkout of `--original-head-sha`, then copying `untracked/` back in, reproduces the worktree's exact state. `PRESERVATION_FAILED` is raised instead of a success response when the untracked enumeration itself failed (`untracked_file_enumeration_failed`, which also covers a `--path` that is not a git worktree at all), when HEAD moved but no patch could be captured (`committed_changes_present_but_patch_capture_failed`), when a copied untracked file's size does not match its source (`untracked_copy_incomplete:<path>`, which for a symlink compares link targets rather than sizes), or when any path `git status` reported as untracked was not copied at all (`untracked_entry_not_preserved:<path>` — an unreadable file, a nested repository directory, anything the copy could not handle); the error carries the `problems` list and `worktree_path`, and the worktree is left untouched. **Do not call `worktree cleanup` after that error** — stop and report the block. `WORKTREE_NOT_FOUND` means `--path` is not a directory at all.
...
`committed_paths` is read back from the sealed commit itself (`git diff-tree --root` against `commit_sha`), not from the pre-commit staged set, so it reflects what actually landed. A stray unrelated staged file in the same worktree produces `STAGED_SET_MISMATCH` instead, with `staged` and `intended` arrays showing exactly where they diverge, before any commit is made. Both the `git add` and the `git commit` run under `-c core.hooksPath=<empty dir>`, so no repository hook of any kind (`pre-commit`, `prepare-commit-msg`, `commit-msg`, `post-commit`) executes on this path. Two post-commit checks remain as defense in depth against any other source of index movement: `COMMIT_FILE_SET_DRIFTED_FROM_STAGED` (with `committed`, `intended`, `commit_sha`) when the sealed commit's file set no longer matches the intended path list, and `COMMIT_CONTENT_DRIFTED_FROM_STAGED` (with `drifted_paths`, `commit_sha`) when the file set still matches but an intended file's blob hash no longer equals the one staged a moment earlier. Either way, revalidate against the committed tree before publishing; the mechanical check reports that content moved, not whether the moved content is still correct.
...
Treat the fix as published only when `matches_pushed_commit` is `true`; it requires both `remote_head_after` (from `git ls-remote` against `--head-remote-url`) and `pr_head_after` (a fresh `repos/{repo}/pulls/{pr}` read) to equal `--commit-sha`, since `remote_head_after` reads back the same URL the push used and therefore agrees with itself by construction. When either post-push read fails or returns nothing usable, the response is still `ok: true` with that field `null`, `verification_incomplete: true`, and `matches_pushed_commit: false`: the push is a completed, irreversible fact and is reported as one, while the missing confirmation is reported as missing rather than raising a `SUBPROCESS_FAILED` that would hide the completed mutation. `push_landed` reflects only the `ls-remote` answer, so check `verification_incomplete` before reading anything into it. Do not retry the push on that response, ever, and there is no automatic retry inside the helper.
...
```json
{
  ...
  "lock_token": "3f9c1a7d4b2e08c65d1f8a03b7e42c91"
}
```

`lock_token` is a fresh random value per acquire, and the only proof of ownership a later `state unlock` accepts. Hold it in the run's own context and pass it back in Phase H; do not write it to a file, and do not read it back out of the lock file, which would defeat the point of having it.

Read any prior record before deciding whether this is a fresh run or a resume (returns `exists: false` with no `record` key when nothing has been written yet, and creates nothing on disk, which is why this one verb is allowed under `--dry-run`):
...
Release the lock, always, even on an early or blocked exit, with the token the acquire returned:

```bash
python3 "$CLAUDE_PLUGIN_ROOT/examples/triage/scripts/triage_helper.py" state unlock \
  --repo-root "$REPO_ROOT" --repo atg/platform-skills --pr 482 \
  --lock-token 3f9c1a7d4b2e08c65d1f8a03b7e42c91
```
...
Omit `--lock-token`, or pass one that does not match the lock file's recorded token, and the release is refused with `LOCK_TOKEN_MISMATCH` and the lock file is left in place. That refusal is the point: it is what stops a second run from releasing a lock it never acquired.

A second `state lock` call for the same repo/PR while the first is still held returns `LOCK_HELD` with `lock_path`, `held_by_pid`, `held_since`, and `age_seconds`, not a queued wait. Clearing a lock once you are confident the run that acquired it has ended is `state unlock --force-unlock`, which needs no token (see the local-lock section above for why that judgment is made from `age_seconds`, not from `held_by_pid`).
...
| Calling `snapshot` more than once in a run | Doubled API cost, and a second reading of live state that may disagree with the first rather than confirm it | Snapshot once, reuse `map-thread` against that one snapshot |
...
| Running `worktree cleanup` before `state write` | `worktree remove --force` drops the worktree's HEAD; a local-only commit becomes unreachable | `worktree preserve`, `state write`, then clean up, on failure paths too |
| Capturing a failed run with `git diff HEAD` instead of `worktree preserve` | Untracked files never appear in a diff, so a new implementation or test file is destroyed silently by the cleanup | `worktree preserve` captures committed, uncommitted, and untracked work and verifies the capture |
| Running `worktree cleanup` anyway after `PRESERVATION_FAILED` | The one error that says "the work could not be safely captured" is followed by the command that destroys it | Stop, leave the worktree in place, report the block |
| Passing a `--head-remote-url` that is not the PR's own head repository | The push lands in an unrelated repository, irreversibly, and only shows up as `matches_pushed_commit: false` afterwards | `publish` refuses with `PUBLISH_DESTINATION_MISMATCH` before pushing; derive the URL from `resolve-identity`'s `head_repo` |
| Releasing the lock with a plain `state unlock` from a run that did not acquire it | Mutual exclusion breaks: two runs mutate one PR at once | Pass the `--lock-token` from this run's acquire; `--force-unlock` only for a confirmed-dead run |
...
| Using `✅ Fixed` when required checks are failing or undiscoverable, without saying so | Reviewer believes a fix is fully verified when it is not | Reserve the suffix for `matches_pushed_commit: true`; a truthfully disclosed pending check does not disqualify it, a failing or undiscoverable one does |
