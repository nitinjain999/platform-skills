...
`worktree preserve` verifies before it reports success: if HEAD moved past `--original-head-sha` but no patch could be produced, or any copied untracked file's size does not match its source, it raises `PRESERVATION_FAILED` with a `problems` list and leaves the worktree untouched. **On `PRESERVATION_FAILED` the agent must not call `worktree cleanup` at all** — it stops, reports the block, and leaves the worktree in place for manual recovery. Losing the run is recoverable; losing the work is not. Getting this order or this gate wrong is what silently turns the preservation promises in the recovery table below into a lie.
