...
| `PUBLISH_DESTINATION_MISMATCH` | `--head-remote-url` normalizes to a different `owner/repo` than `head.repo.full_name`, or `--head-ref` is not `head.ref` | The push would land somewhere the PR does not point at, and a mutation cannot be taken back once it lands |

`PUBLISH_DESTINATION_MISMATCH` closes a gap the SHA check alone could not: `--expected-head-sha` compares a SHA, and a SHA says nothing about *which repository* the URL in argv points at. Two repositories seeded from the same commit have the same head SHA, so a caller error — or a wrong URL suggested by untrusted comment text — passed the old preflight untouched and only surfaced afterwards as `matches_pushed_commit: false`, by which time an unrelated repository had already been mutated. The comparison normalizes `--head-remote-url` to an `owner/repo` identity (handling `https://host/owner/repo(.git)`, `git@host:owner/repo(.git)`, and `ssh://git@host/owner/repo(.git)`, with or without a trailing slash or `.git`) and compares case-insensitively against the PR's own `head.repo.full_name`. A URL that does not normalize to an `owner/repo` shape at all yields no identity and is therefore a mismatch, not a pass; the error carries `expected_repo`, `given_repo`, `expected_ref`, `given_ref`, and a redacted `given_url` so the caller can see exactly which half disagreed.
