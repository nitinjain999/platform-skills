...
| Symptom | Cause | Fix |
|---|---|---|
| `rendered manifests contain a resource that already exists` / `cannot be imported into the current release: invalid ownership metadata` | **Ownership conflict** — a ClusterRole/CRD/resource already exists in the cluster and was not created by this Helm release (e.g. leftover from a previous manual install). Layer: **chart rendering**. Evidence: `flux logs --kind=HelmRelease --name=<name> -n <ns>`, `kubectl get clusterrole <name>`, `helm list -n <ns>`, `kubectl get events -n <ns>`. Root cause: Helm cannot adopt a resource it did not create. Fix: `helm uninstall <release> -n <ns>` then `flux reconcile helmrelease <name> -n <ns>` — or annotate the resource with `meta.helm.sh/release-name` and `meta.helm.sh/release-namespace` to adopt it. Blast radius: `helm uninstall` deletes all Helm-managed resources including CRDs, causing a brief certificate issuance lapse for cert-manager. Validation: `flux get helmrelease <name> -n <ns>` shows `Ready=True`; `kubectl get pods -n <ns>` shows all pods Running. |
