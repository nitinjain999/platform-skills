...
```
`head -3` is required, not cosmetic — `.agent.md` and `.mdc` carry the marker on line 2, inside the frontmatter. See `references/setup-agents-schemas.md` → "Managed-file marker".
...
| `agents/openai.yaml` (tooling metadata, not Codex config) | `assets/frontmatter/codex.tmpl` | `DISPLAY_NAME`, `SHORT_DESCRIPTION`, `DEFAULT_PROMPT`, `AGENT_ENTRIES` |
...
**Prune `AGENTS.md`'s "How to invoke agents" section to the confirmed roster.** The rendered template includes a subsection for every tool surface, regardless of which ones this run actually generated. After rendering, delete any subsection whose tool target is not in the confirmed manifest for this run:

| Subsection | Keep only if manifest contains |
|---|---|
| `### VS Code (GitHub Copilot Chat)` | `copilot-vscode` or `copilot-cloud` |
| `### Cursor` | `cursor` |
| `### Claude Code` | always — `AGENTS.md` is its entry point regardless of roster |
| `### Codex` | always — Codex discovers `AGENTS.md` natively, so it is always supported |

Pruning matters because a leftover subsection can reference a file this run never generated — a dead reference the staleness guard above would otherwise catch.

`### Codex` is generated-file-independent: it describes the `AGENTS.md` root-walk, names no roster file, and is therefore always accurate. Do not gate it on the `codex` token — that token only governs whether `agents/openai.yaml` (setup-agents tooling metadata) gets written.
...
The script reads two inputs, and both must be accurate or checks silently weaken:

- `.platform-skills/manifest` — which tool targets to check at all
- the `## Agent roster` table in `AGENTS.md` — **which agents** to expect per target

The roster table drives per-agent file checks (`.github/agents/<role>.agent.md`, `.cursor/rules/<role>.mdc`). If the table cannot be parsed the script says so and degrades to presence-only checks, which pass on a single surviving file. So keep the roster table intact and machine-readable: one row per agent, role slug in the first column.

Write the manifest now, one token per line matching the confirmed output roster:
...
Valid tokens: `copilot-vscode`, `copilot-cloud`, `copilot-app`, `cursor`, `claude`, `codex`, `windsurf`, `vscode-mcp`. Write `claude` whenever this run appended the `## Agent Context` section to `CLAUDE.md` — without the token, verification cannot tell a deleted `CLAUDE.md` from a repo that never had one. See `references/setup-agents.md` → ".platform-skills/manifest format" for the full reference.
...
if [ -z "$MANIFEST_TARGETS" ]; then
  ...
  grep -q '## Agent Context' CLAUDE.md 2>/dev/null && MANIFEST_TARGETS="$MANIFEST_TARGETS claude"
...
# No per-agent files: the openai.yaml roster metadata holds every agent — read it whole
