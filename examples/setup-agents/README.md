...
**Generated files (all carry the `generated by platform-skills` marker):**
```
...
agents/openai.yaml                   — roster + model record (setup-agents metadata, not Codex config)
scripts/verify-agents.sh
```

**`platform.mdc` (excerpt)** — note `---` on line 1 and the marker as a YAML comment inside the frontmatter, so Cursor can still read `description` and `globs`:
```markdown
---
