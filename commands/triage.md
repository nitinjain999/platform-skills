...
- **A: identity.** `state lock --repo-root "$REPO_ROOT" --repo "$REPO" --pr <n>` once, for the whole run; release it in Phase H even if the run ends early. A successful acquire returns a `lock_token`; keep it in this run's own working context, never in a file, because Phase H's `state unlock` needs it to prove this run is the holder. Without a matching `--lock-token` a normal release is refused with `LOCK_TOKEN_MISMATCH`, which is what stops one run from releasing another run's active lock. Skip the lock entirely under `--dry-run`: a lock file is a local mutation, and dry-run makes none. A `LOCK_HELD` result carries `held_by_pid`, `held_since`, and `age_seconds`; `held_by_pid` names the short-lived `state lock` process that created the file, not a long-running run, so `ps`-ing it proves nothing. Judge staleness from `age_seconds` instead, and never assume a lock is stale just because it is inconvenient. `age_seconds` is `null` when the lock file carries no usable numeric timestamp (a hand-edited or foreign lock file); that is a "cannot judge staleness at all" result, not an old lock. `resolve-identity --repo "$REPO" --pr <n>` once. A draft PR is a valid open PR, not a blocker; only `state != open` refuses the run. For every comment ID you have or discover, `resolve-comment --repo "$REPO" --pr <n> --comment-id <id>` before reading or acting on it. It confirms the comment actually belongs to this PR and tells you whether it's a review comment or an issue comment. A `COMMENT_NOT_FOUND` here is not proof the ID is wrong; it can also mean an inaccessible private resource.
- **B: snapshot.** `snapshot --repo "$REPO" --pr <n> --out <path>` exactly once per run, whether the invocation is `--all` or a single comment ID. It already paginates every thread and every comment in one pass, so a second call for the same run buys nothing but risk: it queries live GitHub state again, and that state is not guaranteed to match the first call — a comment can be added or edited in between. Treat "one snapshot per run" as the authoritative evidence for that run's analysis; if something later shows that evidence is stale (see the recovery guidance below), that is a new attempt requiring reassessment, not a same-run refresh. Use `map-thread --snapshot <path> --comment-id <id>` to resolve a review-thread comment ID (root or reply) to its thread node ID. `map-thread` covers review comments only, the ones `resolve-comment` reported as `comment_type: "review"`; a snapshot holds review threads and nothing else. A conversation comment (`comment_type: "issue"`) is deliberately not in it, so never send one through `map-thread` — that raises `COMMENT_NOT_IN_SNAPSHOT`, which here would be a self-inflicted hard stop, not evidence about the comment. That comment goes straight to Phase G's `reply --pr <n>` path with no thread ID and no resolution step.
...
- **D: isolate.** Before editing anything, `worktree prepare --repo-root "$REPO_ROOT" --head-sha <verified_head_sha> --fetch-remote-url <derived from resolve-identity's host/head_repo, e.g. https://<host>/<head_repo>.git>`. The head SHA is commonly absent from a base-repo checkout, fork PRs especially, so `--fetch-remote-url` fetches it first; if a clean remote URL can't be derived, omit the flag; it's optional and `worktree prepare` still works when the SHA is already local. Never edit the caller's own checkout. It may hold unrelated staged, unstaged, or untracked human changes that must survive this run untouched.
- **E: validate, then commit.** Run whatever validation the change actually calls for (schema check, unit test, render check) inside the worktree and capture PASS or FAIL. Only on a reported PASS: `stage-commit --worktree <dir> --paths <file...> --message "<msg>"`. It refuses to commit when the staged file set doesn't exactly match `--paths`, which is what catches cross-file contamination (a leftover `git add` from something else). It cannot detect an unrelated edit sitting inside one of those same files; what protects against that is Phase D's isolation, since a fresh detached worktree holds no pre-existing human edits for `git add -- <path>` to sweep in. Both the `git add` and the `git commit` behind this subcommand run with hooks disabled, so a repository `pre-commit`, `prepare-commit-msg`, `commit-msg`, or `post-commit` hook never executes here. That is not only about content drift: such a hook runs arbitrary code with this run's credentials, and a hook that makes a network call or reads an unrelated file without touching the staged bytes leaves no drift for any check to observe. If a repository genuinely wants hook-driven formatting or linting as part of a fix, that has to be an explicit, separately invoked step, never something this path triggers implicitly. After the commit it still rereads the sealed commit and raises `COMMIT_FILE_SET_DRIFTED_FROM_STAGED` (the commit's file set no longer matches the intended paths) or `COMMIT_CONTENT_DRIFTED_FROM_STAGED` (an intended file's blob hash moved) as defense in depth against any other source of index movement. Either one means the validation you just ran does not describe what landed: re-inspect the commit and revalidate before Phase F.
- **F: publish.** `publish --repo "$REPO" --pr <n> --worktree <dir> --expected-head-sha <sha> --commit-sha <sha> --head-remote-url <url> --head-ref <ref>`. Before it pushes anything it reads the PR once and refuses five ways: `PR_READ_UNUSABLE` (that read returned no usable JSON, so nothing downstream is verified), `PR_NOT_OPEN` (the PR closed or merged since the plan was built), `HEAD_MOVED` (the head advanced), `HEAD_REPO_UNAVAILABLE` (the head repository is `null`, a deleted or inaccessible fork, so the destination cannot be verified at all), and `PUBLISH_DESTINATION_MISMATCH` (`--head-remote-url` resolves to a different host or a different `owner/repo` than the PR's real head repository, or `--head-ref` is not the PR's real head branch — the host half matters, since `https://evil.example.com/acme/widgets.git` carries the same `owner/repo` tail as the real one, and a local or relative path with no host at all is a mismatch rather than a pass). That last one is why a wrong destination is now a refusal rather than an after-the-fact report: a push is irreversible, and `matches_pushed_commit: false` arrives too late to undo one. It also refuses to force-push. Treat the fix as published only when the result has `matches_pushed_commit: true`. `verification_incomplete: true` is a third outcome, not a failure: the push completed but at least one of the two post-push reads never answered, so `remote_head_after` or `pr_head_after` is `null`. Do not retry the push in that state — it already happened, and a retry would come back `HEAD_MOVED`. Read the head yourself, then decide.
- **G: reply, then resolve.** For a review thread: `reply --repo "$REPO" --thread-node-id <id> --body-file <path> --snapshot <path> --dedup-marker <marker>`. For a PR conversation comment: `reply --repo "$REPO" --pr <n> --body-file <path>` (no `--thread-node-id`, no `--snapshot`, no `--dedup-marker` — the helper refuses those with `INVALID_ARGUMENTS` on this path because nothing checks them here, so this reply is not idempotent and a blind retry double-posts; there's also no thread to resolve, so stop after the reply). A `status: ALREADY_REPLIED` means don't post again. Only when the finding is eligible (below), `resolve-thread --thread-node-id <id> --snapshot <path> --allow-new-comment-node-id <the comment_node_id the reply just returned>`, and read its `status` (`CONFIRMED` or `ALREADY_RESOLVED`) as the only proof of closure. Passing `--snapshot` is optional but recommended: it compares the thread's live comment node IDs against the set the snapshot recorded and refuses with `THREAD_CHANGED_SINCE_SNAPSHOT` when any ID appeared or vanished, which is exactly the "no new human input" eligibility check below. `--allow-new-comment-node-id` is what keeps that guard from tripping on this run's own reply: `reply`'s thread path returns the new comment's node ID in `comment_node_id`, and passing it here says "this one addition is mine, anything else is someone else's." Repeat the flag once per allowed ID. Omit `--snapshot` and both the guard and the flag are inert. When `reply` short-circuited to `status: ALREADY_REPLIED`, `comment_node_id` comes back `null`; omit `--allow-new-comment-node-id` entirely in that case rather than passing the literal string `null` — a bogus value there is harmless (it just never matches a live ID) but omitting it is cleaner. A `--snapshot` naming a snapshot with no entry for `--thread-node-id` fails closed with `THREAD_NOT_IN_SNAPSHOT` before any API call; take that as "get a fresh snapshot that actually covers this thread," not as license to retry with `--snapshot` omitted just to get past it.
- **H: persist, report, then clean up.** In this order, on every path, including every failure path:
  1. If a worktree exists and the run did not end in a confirmed publish (validation failed, `stage-commit` failed, `publish` was rejected, or anything aborted earlier), capture what that worktree holds **before** destroying it: create a temp directory yourself, then `worktree preserve --path <dir> --original-head-sha <prepared_head_sha> --out <temp dir>`. It writes `committed.patch` for anything committed past the prepared head, `uncommitted.patch` (with `--binary`, so binary changes survive) for edits on top, and byte-for-byte copies of every untracked file under `untracked/`. That last channel is the one plain `git diff HEAD` never had: a brand-new implementation or test file that was never `git add`ed is invisible to a diff and would be destroyed by the forced removal in step 3. Record the returned paths. If it raises `PRESERVATION_FAILED`, do **not** continue to step 3 at all: stop, report the block, and leave the worktree in place for manual recovery.
  2. Write what has to survive the run (finding verdicts, execution state, discussion state, and the preserved paths from step 1) as a small JSON object to a temp file you create yourself, then `state write --repo-root "$REPO_ROOT" --repo "$REPO" --pr <n> --record-file <path>`; the record's actual fields are documented in `references/triage.md`'s data model, not here.
  3. `worktree cleanup --repo-root "$REPO_ROOT" --path <dir>`. Never run this before step 2, and never at all when step 1 reported `PRESERVATION_FAILED`: `worktree remove --force` deletes the worktree's `.git/worktrees/<id>` along with its HEAD and reflog, so a commit that existed only there becomes unreachable and collectable the moment it goes, and it deletes the working directory outright, taking untracked files with it. That is exactly the work the recovery table promises to preserve.
  4. `state unlock --repo-root "$REPO_ROOT" --repo "$REPO" --pr <n> --lock-token <the token Phase A returned>`. Without the token this refuses with `LOCK_TOKEN_MISMATCH`; `--force-unlock` is the only bypass and is for clearing a confirmed-dead run's lock, not for skipping bookkeeping.
...
- `--dry-run`: run `resolve-identity`, `resolve-comment`, `snapshot`, and `patch-context` only. Print the classification, the evidence, and the proposed `stage-commit`/`publish`/`reply`/`resolve-thread` plan. Never call `worktree prepare`, `worktree preserve`, `worktree cleanup`, `stage-commit`, `publish`, `reply`, `resolve-thread`, `state lock`, `state unlock`, `state write`, or the closing self-improve log. A lock file and a state record are mutations too, small and local rather than remote, and dry-run makes none of any kind. `state read` is fine: it only reads, and it does not create `.git/triage-state/` as a side effect when nothing has been written there yet.
...
- `ACTIONABLE_FIX`, once `publish` has confirmed `matches_pushed_commit: true` **and** required checks are not failing (a check still pending is fine as long as the reply says so; a failing check, or a required-check state that could not be discovered at all, is not): one sentence naming the file and what changed. The literal string `✅ Fixed` is the last thing in the body, with nothing after it. Never use this suffix on a failed, local-only, or unvalidated change.
...
- Never call `worktree cleanup` before `state write`, and never on a failure path before `worktree preserve` has returned `ok: true`. The removal is what destroys the evidence, and it takes untracked files with it.
- Never call `worktree cleanup` after `worktree preserve` raised `PRESERVATION_FAILED`. Leave the worktree in place and report the block instead.
- Never rely on `git diff HEAD` alone to capture a worktree's work. It never shows untracked files, so a new implementation or test file would be lost silently. Use `worktree preserve`.
- Never release a lock with `--force-unlock` just because the `--lock-token` was mislaid. Force is for a lock whose acquiring run is confirmed ended, judged from `age_seconds`, not a shortcut around the ownership check.
...
- Always release the `state lock` in Phase H with the `--lock-token` Phase A returned, even when the run ends early or blocked. There is nothing to release under `--dry-run`, which never takes one.
- Always run `worktree preserve` then `state write` before `worktree cleanup`, on success and on failure alike, so a commit, an uncommitted edit, or an untracked file that only exists in the worktree is captured before the worktree is removed.
