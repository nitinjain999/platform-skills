...
- **E: validate, then commit.** Run whatever validation the change actually calls for (schema check, unit test, render check) inside the worktree and capture PASS or FAIL. Only on a reported PASS: `stage-commit --worktree <dir> --paths <file...> --message "<msg>"`. It refuses to commit when the staged file set doesn't exactly match `--paths`, which is what catches cross-file contamination (a leftover `git add` from something else). It cannot detect an unrelated edit sitting inside one of those same files; what protects against that is Phase D's isolation, since a fresh detached worktree holds no pre-existing human edits for `git add -- <path>` to sweep in. After the commit it rereads the sealed commit and raises `COMMIT_FILE_SET_DRIFTED_FROM_STAGED` (a hook added or dropped a file) or `COMMIT_CONTENT_DRIFTED_FROM_STAGED` (a hook rewrote an intended file's bytes, so the blob hash moved). Either one means the validation you just ran does not describe what landed: re-inspect the commit and revalidate before Phase F.
- **F: publish.** `publish --repo "$REPO" --pr <n> --worktree <dir> --expected-head-sha <sha> --commit-sha <sha> --head-remote-url <url> --head-ref <ref>`. It rechecks the PR head before pushing and refuses to force-push. Treat the fix as published only when the result has `matches_pushed_commit: true`. `push_landed: true` with `verification_incomplete: true` is a third outcome, not a failure: the commit is on the head ref (`git ls-remote` agrees) but the post-push `pulls/{pr}` read never answered, so `pr_head_after` is `null`. Do not retry the push in that state — it already happened, and a retry would come back `HEAD_MOVED`. Read the PR head yourself, then decide.
