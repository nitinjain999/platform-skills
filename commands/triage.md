...
- **H: report and clean up.** `worktree cleanup --repo-root "$REPO_ROOT" --path <dir>`. Write whatever you need to persist (finding verdicts, execution state, discussion state) as a small JSON object to a temp file you create yourself, then `state write --repo-root "$REPO_ROOT" --repo "$REPO" --pr <n> --record-file <path>`; the record's actual fields are documented in `references/triage.md`'s data model, not here. Then `state unlock --repo-root "$REPO_ROOT" --repo "$REPO" --pr <n>`. Then the capability-aware closing step described below.
