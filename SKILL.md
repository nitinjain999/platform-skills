...
- `/platform-skills:token-optimizer` — route broad repository discovery to a cheaper worker model using each client's native subagents; `inspect` detects clients and conflicts, `setup` writes a reviewable diff, `doctor` reports delegation, resolved model, redirection, and read limit as four separate states, `explain` dry-runs a classification, `benchmark` and `report` measure total cost across parent and worker
